...
function TopNav() {
  ...
  const [dashboardBlueCount, setDashboardBlueCount] = useState(0);
  const dashboardTimers = useRef([]);

  const dashboardChars = "Dashboard".split("");

  const clearDashboardTimers = () => {
    dashboardTimers.current.forEach(clearTimeout);
    dashboardTimers.current = [];
  };
  ...
  const animateDashboardLetters = () => {
    clearDashboardTimers();
    setDashboardBlueCount(0);

    const totalLetters = dashboardChars.length;
    const totalDuration = 200; // slower, visible letter-by-letter change
    const stepDuration = totalDuration / totalLetters;

    dashboardChars.forEach((_, index) => {
      const timer = setTimeout(() => {
        setDashboardBlueCount(index + 1);
      }, stepDuration * index);

      dashboardTimers.current.push(timer);
    });
  };
  ...
  const translateX = underlineAnim.interpolate({
    ...
    outputRange: ["-10%", "0%", "100%", "200%", "300%", "400%"],
  });

  return (
    <View style={styles.frame155}>
      <View style={styles.topNav}>
        <Pressable style={styles.navTab} onPress={() => goToTab(0, "/home")}>
          <View style={styles.dashboardLetterRow}>
{dashboardChars.map((char, index) => {
  const fromRightIndex = dashboardChars.length - 1 - index;
  ...
  return (
    <Text
      ...
      style={[
        ...
        fromRightIndex < dashboardBlueCount
    ...
    >
      {char}
        ...
        <View style={styles.navTab}>
          <Text style={activeTab === 1 ? styles.navItemActive : styles.navItem}>
            Debt
          </Text>
        </View>

        <Pressable style={styles.navTab} onPress={() => goToTab(2, "/spending")}>
          <Text style={activeTab === 2 ? styles.navItemActive : styles.navItem}>
            Spending
          </Text>
        </Pressable>

        <Pressable style={styles.navTab} onPress={() => goToTab(3, "/credit")}>
          <Text style={activeTab === 3 ? styles.navItemActive : styles.navItem}>
            Credit
          </Text>
        </Pressable>

        <Pressable style={styles.navTab} onPress={() => goToTab(4, "/accounts")}>
          <Text style={activeTab === 4 ? styles.navItemActive : styles.navItem}>
            Accounts
          </Text>
        </Pressable>
      </View>
